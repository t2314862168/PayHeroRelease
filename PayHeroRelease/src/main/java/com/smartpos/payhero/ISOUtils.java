...
//    		throw new IllegalArgumentException("only support 3 bytes tag" + Dump.getHexDump(ISOUtils.intToBytes(rslt, 4, true)));
//    	}
//    	if(rslt > 0xFFFF){ //长度为3个字节，第一个字节必须是xxx11111,第二个字节必须高位为1，才可能存在第三个字节,第三字节高位必须为0
//    		if((rslt & 0x1F8000) != 0x1F8000 || ((rslt & 0x80) == 0x80 )){
//    			throw new IllegalArgumentException("illegal tag format:"+Dump.getHexDump(ISOUtils.intToBytes(rslt, 3, true)));
//    		}
//    		//满足条件返回结果
//    		return rslt;
//    	}
...
//    			throw new IllegalArgumentException("illegal tag format:"+Dump.getHexDump(ISOUtils.intToBytes(rslt, 2, true)));
//    		}
//    		//满足条件返回结果
//    		return rslt;
//    	}
//    	if((rslt & 0x1F) == 0x1F){ //长度仅为1个字节
//    		throw new IllegalArgumentException("illegal tag format:"+Dump.getHexDump(ISOUtils.intToBytes(rslt, 1, true)));
//    	}
//    	return rslt;
//    }
    /**
